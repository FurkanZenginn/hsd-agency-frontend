...
export const Booking = ({ isDashboard = false, onBookingComplete }) => {
    ...
    // API'den gelen veriler (mock modda servis dosyalarındaki mock veri döner)
    ...
    useEffect(() => {
        staffService.getStaff()
            .then((data) => { if (Array.isArray(data)) setStaffMembers(data); })
            .catch(() => { });
    }, []);

    // Seçili personel + tarih değişince müsait saatleri çek
    useEffect(() => {
        if (bookingData.staff?.id && bookingData.date) {
            setSlotsLoading(true);
            staffService.getAvailableSlots(bookingData.staff.id, bookingData.date)
                .then((data) => { if (Array.isArray(data)) setTimeSlots(data); })
                .catch(() => { })
                .finally(() => setSlotsLoading(false));
        }
    }, [bookingData.staff?.id, bookingData.date]);
    ...
    const handleConfirm = async () => {
        ...
        try {
            await addAppointment(bookingData);
            setIsConfirmed(true);
        } catch (err) {
        ...
        return (
            <div className={isDashboard ? "px-4 py-8 text-center" : "max-w-3xl mx-auto px-4 py-32 text-center animate-fade-in-up"}>
                <div className={`${isDashboard ? 'bg-transparent text-dark' : 'bg-white text-dark shadow-2xl border border-gray-100'} rounded-3xl p-8 md:p-16 flex flex-col items-center`}>
                    <div className={`h-24 w-24 rounded-full flex items-center justify-center mb-8 border bg-green-50 text-green-500 border-green-100`}>
                    ...
                    <div className="border border-gray-100 p-8 rounded-2xl w-full max-w-md mb-10 text-left space-y-4 bg-gray-50/50">
                        ...
                        <div key={i} className="flex items-center">
                            ...
                            {i < 3 && (
                                <div className="w-16 sm:w-24 px-2">
                                    <div className={`h-[2px] w-full rounded-full transition-all duration-500 ${step > i ? 'bg-primary-500' : 'bg-gray-200'}`}></div>
                                </div>
                            )}
                        </div>
                    ))}
            ...
            <div className={`${isDashboard ? 'bg-transparent' : 'bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100'} min-h-[450px] relative overflow-hidden ${isDashboard ? '' : 'rounded-none sm:rounded-3xl p-8 md:p-14'}`}>
                ...
                {step === 1 && (
                    <div className="animate-fade-in">
                        ...
                        {services.length === 0 ? (
                            <p className="text-center text-gray-400">Hizmetler yükleniyor...</p>
                        ) : (
                ...
                {step === 2 && (
                    <div className="animate-fade-in">
                        ...
                        {staffMembers.length === 0 ? (
                            <p className="text-center text-gray-400">Uzmanlar yükleniyor...</p>
                        ) : (
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                                {staffMembers.map(staff => (
                                    <button
                                        ...
                                        className={`text-center p-8 rounded-2xl border transition-all duration-300 group ${bookingData.staff?.id === staff.id ? 'border-primary-500 shadow-md ring-1 ring-primary-500 bg-primary-50/50' : 'border-gray-200 hover:border-primary-300 hover:shadow-md bg-white'}`}
                                    >
                    ...
                    <div className="animate-fade-in">
                        <div className="grid md:grid-cols-2 gap-12">
                            ...
                            <div>
                                ...
                                {slotsLoading ? (
                                    <p className="text-gray-400 text-sm">Müsait saatler yükleniyor...</p>
                                ) : (
                                    <div className="grid grid-cols-2 gap-4">
                                        {timeSlots.map(time => (
                                            <button
                                                ...
                                                className={`py-4 px-3 rounded-xl text-sm font-medium tracking-wider transition-all duration-300 border ${bookingData.time === time ? 'bg-primary-500 text-white border-primary-500 shadow-md shadow-primary-500/20' : 'bg-white text-dark border-gray-200 hover:border-primary-500 hover:text-primary-600'}`}
                                            >
                                                {time}
                                            </button>
                                        ))}
                ...
                <div className="mt-12 flex justify-between border-t border-gray-100 pt-6">
                    ...
                    {step < 3 ? (
                        <Button variant="primary" onClick={handleNext} disabled={(step === 1 && !bookingData.service) || (step === 2 && !bookingData.staff)}>
                            Devam Et
                        </Button>
                    ) : (
                        <Button variant="primary" onClick={handleConfirm} disabled={!bookingData.date || !bookingData.time || isSubmitting}>
